...
let nightMode = document.getElementById("toggleNightMode");
let body = document.body;
let antButton = document.getElementById("prevButton");
let proxButton = document.getElementById("nextButton")
let count = 0;

nightMode.addEventListener("click", event =>{
    body.classList.toggle("dark")
    if(count % 2 == 0) {
        nightMode.innerText = "Modo Diurno"

        antButton.classList.remove("is-dark");
        antButton.classList.add("is-light");

        proxButton.classList.remove("is-dark");
        proxButton.classList.add("is-light");
    } else {
        nightMode.innerText = "Modo Noturno"

        antButton.classList.remove("is-light");
        antButton.classList.add("is-dark");

        proxButton.classList.remove("is-light");
        proxButton.classList.add("is-dark");
    }
    count++;
})
...
let arrayImg = ["images/bagmon/1.jpeg",
...
antButton = document.getElementById("prevButton");
proxButton = document.getElementById("nextButton");

let pos = 0;
img.setAttribute("src", arrayImg[pos]);



proxButton.addEventListener("click", event => {
    event.preventDefault();
    if(pos >= 0 && pos < arrayImg.length - 1) {
        pos++;
        img.setAttribute("src", arrayImg[pos])
        img.setAttribute("alt", arrayNames[pos]) // Questão 03
    } else if (pos == arrayImg.length - 1) {
...
antButton.addEventListener("click", event => {
    ...
    if(pos > 0 && pos <= arrayImg.length - 1) {
        ...
        img.setAttribute("src", arrayImg[arrayImg.length - 1]);
