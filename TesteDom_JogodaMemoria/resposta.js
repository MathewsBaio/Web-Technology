...
cards.forEach(card =>{
    card.addEventListener("click", event =>{
        if(count == 1) {
            ...
            card.classList.add("A"); // Questão 04
            ...
            if(arrayLetters[card.getAttribute("posicao")] == arrayLetters[lastcard]){
                ...
                newDupla = true;
                    ...
                    if (!newDupla) {
                    card.textContent = `${arrayLetters[card.getAttribute("posicao")]}` //Correção
                    card.classList.add("B") //Correção
                    count = 3; //Correção
                    setTimeout(resetCards, 2000) // Questão 05
                    }
...
function resetCards() {
    cards.forEach(card => {
        card.textContent = "X";
        card.classList.remove("A") // Questão 04
